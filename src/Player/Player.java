...
public class Player extends Observable {
	...
	public void placeReinforcements(int reinforcements) {
		...
		while (reinforcements != 0) {
			System.out.println("\nPlayer " + name + " has " + reinforcements + " armies left."
	 ...
	 * 
	 * @param currentPlayerId is a integer value which gives the present ID of the
	 *                        player
	 * 
	 * @param fromCountry     is a string value in which a player can mention to
	 *                        move a army from a country
	 * 
	 * @param toCountry       is a string value in which a player can mention to
	 *                        move a army from a country to another country
	 * 
				...
				if (canAttackFromThisCountry(attackfrom)) {
					...
					if (validOpponentCountry(attackfrom, attackat)) {
						...
						do {
							...
							Vector<Integer> attackerDice = rollDice(
									calculateNumberOfDiceAllowed("attacker", attackfrom, attackat));
							Vector<Integer> defenderDice = rollDice(
									calculateNumberOfDiceAllowed("defender", attackfrom, attackat));
							while (!attackerDice.isEmpty() && !defenderDice.isEmpty()) {
								...
								if (attackerDiceValue > defenderDiceValue) {
									...
									if (checkDefenderArmiesNumberZero(attackfrom, attackat)) {
										System.out.println(
												"Enter the number of armies you would like to place in your new territory:");
										moveArmiesToNewTerritory(attackfrom, attackat, keyboard.nextInt());
										// remove this territory from the players list
										for (Player player : Main.players) {
											// VERIFY IF THE TERRITORY IS REMOVED ONLY FROM THE LOST DEFENDERS
											// TERRITORIES LIST
